...
char *translate_DecList(Node *tree_node) {
    ...
    if (find_brother(varDec, _ASSIGNOP) != NULL) {
        char *exp = translate_Exp(find_brother(varDec, _Exp), varName);
        sm = strcatm(sm, "\n", exp);
    }
...
char *translate_funcCall(Node *tree_node, char *place) {
    ...
    ArgsList *al = calloc(1, sizeof(al));
...
char *translate_Args(Node *tree_node, ArgsList *argsList) {
    ...
    char *t1 = new_temp();
    char *s1 = translate_Exp(tree_node, t1);
    ArgsList *newArg = calloc(1, sizeof(newArg));
...
char *translate_AssExp(Node *tree_node, char *place) {
    ...
    char *t1 = new_tmp();
    char *t2 = new_tmp();
    char *s1 = get_arrLocation(tree_node, t1);
    char *s2 = translate_Exp(tree_node->brother->brother, t2);
    char *st = strcatm("*", "", t1);
...
char *translate_CulExp(Node *tree_node, char *place) {
    ...
    char *t2 = new_tmp();
    char *s1 = translate_Exp(tree_node, t1);
    char *s2 = translate_Exp(tree_node, t2);
