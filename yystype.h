...
enum type_t {
    ...
    _ASSIGNOP, _AND, _OR, _RELOP, _PLUS, _MINUS, _MUL, _DIV, _NOT, _LP, _RP, _LB, _RB, _DOT,
    _STAR, _Program, _ExtDefList, _ExtDef, _ExtDecList, _Specifier, _StructSpecifier, _OptTag,
    _Tag, _VarDec, _FunDec, _VarList, _ParamDec, _CompSt, _StmtList, _Stmt, _DefList, _Def,
    _DecList, _Dec, _Exp, _Args, _OTHER,
};
...
#define NODE_INIT(TYPE)         lexprintf(#TYPE" %s\n",yytext);      \
...
#define NODE_RETURN(TYPE)       yylval = node;                      \
                                return TYPE;

#define add_brother(NO)         ptr = NULL;                                             \
...
#define add_father(TYPE)     Node *node = malloc(sizeof(Node));  \
                             node->type = _##TYPE;               \
